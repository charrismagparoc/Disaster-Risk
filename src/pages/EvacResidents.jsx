...
export function EvacuationPage(){
  ...
  const openEdit=(c)=>{setIsEditing(true);setForm({...c});setShowModal(true);setSelected(null);};
  const handleSave=()=>{
    if(!form.name.trim())return;
    if(isEditing&&form.id){updateEvacCenter(form.id,form);}else{addEvacCenter(form);}
    setShowModal(false);
  };
  const updateStatus=(id,s)=>{updateEvacCenter(id,{status:s});setSelected(p=>p?{...p,status:s}:p);};
  ...
  return(
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16,marginBottom:20}}>
        <div className="card" style={{textAlign:'center'}}><div style={{fontSize:28,fontFamily:'var(--font-display)',fontWeight:800,color:'var(--accent-green)'}}>{evacCenters.filter(c=>c.status==='Open').length}</div><div style={{color:'var(--text-secondary)',fontSize:12}}>Open Centers</div></div>
        <div className="card" style={{textAlign:'center'}}><div style={{fontSize:28,fontFamily:'var(--font-display)',fontWeight:800,color:'var(--accent-blue)'}}>{totalOcc}</div><div style={{color:'var(--text-secondary)',fontSize:12}}>Current Evacuees</div></div>
        <div className="card" style={{textAlign:'center'}}><div style={{fontSize:28,fontFamily:'var(--font-display)',fontWeight:800,color:'var(--accent-orange)'}}>{totalCap-totalOcc}</div><div style={{color:'var(--text-secondary)',fontSize:12}}>Remaining Capacity</div></div>
      </div>
          ...
          return(
            <div key={c.id} className="card" style={{cursor:'pointer',position:'relative'}} onClick={()=>setSelected(c)}>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:12,paddingRight:80}}>
                <div><div style={{fontWeight:700,fontSize:15}}>{c.name}</div><div style={{fontSize:12,color:'var(--text-muted)',marginTop:2}}>{c.address}</div></div>
              ...
              <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                {c.facilitiesAvailable.map(f=><span key={f} style={{fontSize:11,color:'var(--text-muted)',background:'var(--bg-deep)',padding:'2px 8px',borderRadius:20,border:'1px solid var(--border)'}}>{f}</span>)}
              </div>
            </div>
          );
        })}
        ...
        <div className="modal-overlay" onClick={()=>setSelected(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'flex',gap:8,marginBottom:16}}>
              {['Open','Full','Closed'].map(s=><button key={s} className={`btn btn-sm ${selected.status===s?'btn-primary':'btn-secondary'}`} onClick={()=>updateStatus(selected.id,s)}>{s}</button>)}
            </div>
        ...
        <div className="modal-overlay" onClick={()=>setShowModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="form-grid">
              ...
              <div className="form-group"><label>Occupancy</label><input className="form-control" type="number" value={form.occupancy} onChange={e=>setForm({...form,occupancy:parseInt(e.target.value)||0})}/></div>
              ...
              <div className="form-group full"><label>Facilities Available</label>
                <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:4}}>
                  {EVAC_FACILITIES.map(f=><button key={f} type="button" className={`btn btn-sm ${form.facilitiesAvailable.includes(f)?'btn-success':'btn-secondary'}`} onClick={()=>toggleFacility(f)}>{f}</button>)}
                </div>
              </div>
            </div>
            <div style={{marginTop:16,display:'flex',gap:8,justifyContent:'flex-end'}}>
              <button className="btn btn-secondary" onClick={()=>setShowModal(false)}>Cancel</button>
              <button className="btn btn-primary" onClick={handleSave}><i className="fa-solid fa-floppy-disk"></i>{isEditing?'Save Changes':'Add Center'}</button>
            </div>
          </div>
        </div>
      )}
...
export function ResidentsPage(){
  ...
  const [deleteId,setDeleteId]=useState(null);
  const [form,setForm]=useState({name:'',zone:'Zone 1',address:'',householdMembers:1,contact:'',evacuationStatus:'Safe',vulnerabilityTags:[]});
  ...
  const openAdd=()=>{setEditing(null);setForm({name:'',zone:'Zone 1',address:'',householdMembers:1,contact:'',evacuationStatus:'Safe',vulnerabilityTags:[]});setShowModal(true);};
  const openEdit=(r)=>{setEditing(r);setForm({...r});setShowModal(true);};
  const handleSave=()=>{
    if(!form.name.trim())return;
    if(editing){updateResident(editing.id,form);}else{addResident(form);}
    setShowModal(false);
  };
  const toggleTag=(t)=>setForm(p=>({...p,vulnerabilityTags:p.vulnerabilityTags.includes(t)?p.vulnerabilityTags.filter(x=>x!==t):[...p.vulnerabilityTags,t]}));
  ...
  return(
    <div>
      ...
      <div className="filter-row">
        <input className="form-control" placeholder="Search resident..." value={search} onChange={e=>setSearch(e.target.value)} style={{maxWidth:240}}/>
        <select className="form-control" value={filterZone} onChange={e=>setFilterZone(e.target.value)} style={{maxWidth:150}}><option>All</option>{ZONES.map(z=><option key={z}>{z}</option>)}</select>
        <select className="form-control" value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} style={{maxWidth:160}}><option>All</option><option>Safe</option><option>Evacuated</option><option>Unaccounted</option></select>
      </div>

      <div className="card" style={{padding:0}}>
        <div className="table-container">
          <table>
            <thead><tr><th>ID</th><th>Name</th><th>Zone / Address</th><th>Household</th><th>Contact</th><th>Vulnerability</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
              {filtered.map(r=>(
                <tr key={r.id}>
              ...
              {filtered.length===0&&<tr><td colSpan={8}><div className="empty-state"><i className="fa-solid fa-users"></i><p>No residents found. Add one above.</p></div></td></tr>}
            </tbody>
          </table>
        </div>
      </div>

      {deleteId&&<ConfirmModal title="Delete Resident" message="Remove this resident from the database?" onConfirm={()=>{deleteResident(deleteId);setDeleteId(null);}} onCancel={()=>setDeleteId(null)}/>}
        ...
        <div className="modal-overlay" onClick={()=>setShowModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="form-grid">
              ...
              <div className="form-group"><label>Zone</label><select className="form-control" value={form.zone} onChange={e=>setForm({...form,zone:e.target.value})}>{ZONES.map(z=><option key={z}>{z}</option>)}</select></div>
              <div className="form-group"><label>Evacuation Status</label><select className="form-control" value={form.evacuationStatus} onChange={e=>setForm({...form,evacuationStatus:e.target.value})}><option>Safe</option><option>Evacuated</option><option>Unaccounted</option></select></div>
              ...
              <div className="form-group"><label>Contact Number</label><input className="form-control" placeholder="09XX-XXX-XXXX" value={form.contact} onChange={e=>setForm({...form,contact:e.target.value})}/></div>
              <div className="form-group full"><label>Vulnerability Tags</label>
                <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:4}}>
                  {VULNERABILITY_TAGS.map(t=><button key={t} type="button" className={`btn btn-sm ${form.vulnerabilityTags.includes(t)?'btn-primary':'btn-secondary'}`} onClick={()=>toggleTag(t)}>{t}</button>)}
                </div>
              </div>
            </div>
            <div style={{marginTop:16,display:'flex',gap:8,justifyContent:'flex-end'}}>
              <button className="btn btn-secondary" onClick={()=>setShowModal(false)}>Cancel</button>
              <button className="btn btn-primary" onClick={handleSave}><i className="fa-solid fa-floppy-disk"></i>{editing?'Save Changes':'Add Resident'}</button>
            </div>
          </div>
        </div>
      )}
